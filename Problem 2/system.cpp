...
*/
void System::Login(string account, string password, campaign& Campaign) {
    if(Map.find(account) != Map.end()) {
        ...
        Profile curUser = profileList[id];
        if(curUser.getPassword() == password) {
            ...
            // If number of voucher is run out, just login and do nothing.
            if(!Campaign.getVoucherLeft()) return;

            if(curUser.firstLogin() == true) { // This user is valid to get a voucher.
                cout << "Congratulations! You have receive a voucher from Cake with discount of " << Campaign.getDiscount() << "%.\n";
                Campaign.releaseVoucher(); // Release voucher code.
                Campaign.add(curUser); // Adding user to have receive the voucher.
            }
            return;
        }
    }
